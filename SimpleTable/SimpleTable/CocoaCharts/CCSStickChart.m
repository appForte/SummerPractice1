...
- (NSString *)calcAxisXGraduate:(CGRect)rect {
    ...
    NSArray* reversedData = [[self.stickData reverseObjectEnumerator] allObjects];
    
    NSString *result = @"";
    if (reversedData != NULL && [reversedData count] > 0) {
        if (self.axisYPosition == CCSGridChartYAxisPositionLeft) {
            if (value >= 1) {
                result = ((CCSStickChartData *) [reversedData objectAtIndex:self.maxSticksNum]).date;
            } else if (value <= 0) {
                result = ((CCSStickChartData *) [reversedData objectAtIndex:0]).date;
            } else {
                NSUInteger index =[self.stickData count] - self.maxSticksNum + (NSUInteger) (self.maxSticksNum * value);
                result = ((CCSStickChartData *) [reversedData objectAtIndex:index]).date;
            }
        } else {
            if (value >= 1) {
                result = ((CCSStickChartData *) [reversedData objectAtIndex:[reversedData count] - 1]).date;
            } else if (value <= 0) {
                result = ((CCSStickChartData *) [reversedData objectAtIndex:[reversedData count] - self.maxSticksNum]).date;
            } else {
                NSUInteger index =  (NSUInteger) (self.maxSticksNum * value);
                if (index > [reversedData count] - 1) {
                    index = [reversedData count] - 1;
                }
                result = ((CCSStickChartData *) [reversedData objectAtIndex:index]).date;
            }
        }
...
- (void)initProperty {
    ...
    self.stickBorderColor = [UIColor blackColor];
    self.stickFillColor = [UIColor blackColor];
    
    self.latitudeNum = 3;
    self.longitudeNum = 3;
    self.maxSticksNum = 200;
    self.maxValue = 200;
    self.minValue = 0;
    self.selectedStickIndex = 0;
...
- (NSString *)calcAxisXGraduate:(CGRect)rect {
    ...
    NSLog(@"MAX STICKS NUM:%i",self.maxSticksNum);
    ...
    if (reversedData != NULL && [reversedData count] > 0) {
        if (self.axisYPosition == CCSGridChartYAxisPositionLeft) {
            ...
            if (value >= 1.168) {
                ...
                NSUInteger index = [reversedData count] - self.maxSticksNum + (NSUInteger) (self.maxSticksNum * value);
                if (index > [reversedData count] - 1) {
                    index = [reversedData count] - 1;
                }
                result = ((CCSStickChartData *) [reversedData objectAtIndex:index]).date;
